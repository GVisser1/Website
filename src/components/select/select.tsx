...
const SelectTrigger = forwardRef<ComponentRef<typeof Radix.Trigger>, Radix.SelectTriggerProps>(
  ({ className, children, ...props }, ref) => (
    <Radix.Trigger
      ref={ref}
      className={clsx(
        "btn-ghost focus-visible:focus-ring flex h-10 w-full items-center justify-between gap-x-2 rounded-sm border border-primary px-3 py-2 text-button dark:border-primary-dark",
        className,
      )}
      {...props}
    >
...
const SelectItem = forwardRef<ComponentRef<typeof Radix.Item>, SelectItemProps>(
  ({ className, children, ...props }, ref) => (
    <Radix.Item
      ref={ref}
      className={clsx(
        "btn-ghost relative flex h-10 w-full select-none items-center truncate rounded-sm py-1.5 pr-8 pl-2 text-button outline-hidden",
